...
class LoggingAgentWrapper:
    ...
    def create_run(self):
        """Calls the /run endpoint to create a new run."""
        print("[WRAPPER] Creating a new run...")
    ...
    def create_task(self, args):
        """Calls the /task endpoint with args."""
        print(f"[WRAPPER] Creating a new task for run_id: {args['run_id']}...")
        response = requests.post(f"{API_URL}/task", json=args)
        if response.status_code == 200:
            task_data = response.json()
            print(
                f"[WRAPPER] Task created successfully with ID: {task_data['task_id']}"
            )
        ...
        for i, state in enumerate(reversed(states[:-1])):
            ...
            if len(curr_messages) > 0:
                ...
                if i == len(states) - 2:  # Last message in the state
                    ...
                    message_type = (
                        self.get_message_type(curr_messages[-1].type)
                        if hasattr(curr_messages[-1], "type")
                        else "unknown"
                    )
                metadata = curr_messages[-1].response_metadata
                tool_calls = (
                    curr_messages[-1].tool_calls
                    if hasattr(curr_messages[-1], "tool_calls")
                    else []
                )
    ...
    def replay(self, args, checkpoint_id=None, thread_id=None, patch=None):
        config = {
            "configurable": {"checkpoint_id": checkpoint_id, "thread_id": thread_id}
        }
